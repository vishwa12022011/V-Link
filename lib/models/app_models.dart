...
class KeyBinding {
  ...
  KeyBinding copyWith({
    String? keySequence, String? label, bool? isHold, bool? isToggle,
  }) => KeyBinding(
        id: id, icon: icon,
...
class HudProfile {
  ...
  HudProfile({
    ...
    required this.bindings, required this.buttons, required this.updatedAt,
...
class WeaponDef {
  ...
  WeaponDef({
    required this.id, required this.name, required this.slotLabel,
    required this.type, required this.keySequence,
  });

  WeaponDef copyWith({String? keySequence}) => WeaponDef(
        id: id, name: name, slotLabel: slotLabel, type: type,
        keySequence: keySequence ?? this.keySequence,
      );
}
...
class WeaponSlotModel extends ChangeNotifier {
  ...
  WeaponSlotModel() { _weapons = _defaults(); }

  static List<WeaponDef> _defaults() => [
    WeaponDef(id:'primary', name:'AK-47',        slotLabel:'Primary',   type:WeaponType.primary, keySequence:'1'),
    WeaponDef(id:'pistol',  name:'Desert Eagle',  slotLabel:'Pistol',    type:WeaponType.pistol,  keySequence:'2'),
    WeaponDef(id:'grenade', name:'Grenade',        slotLabel:'Throwable', type:WeaponType.grenade, keySequence:'4'),
    WeaponDef(id:'knife',   name:'Knife',          slotLabel:'Melee',     type:WeaponType.knife,   keySequence:'3'),
  ];

  WeaponDef       get activeWeapon    => _weapons[_activeIndex];
  List<WeaponDef> get inactiveWeapons => [
    for (int i = 0; i < _weapons.length; i++) if (i != _activeIndex) _weapons[i],
  ];

  void activateById(String id) {
    final idx = _weapons.indexWhere((w) => w.id == id);
    if (idx < 0 || idx == _activeIndex) return;
    _activeIndex = idx; notifyListeners();
  }
...
class ToggleStateModel extends ChangeNotifier {
  ...
  bool flip(String id) { _s[id] = !(_s[id] ?? false); notifyListeners(); return _s[id]!; }
  void setOff(String id) { if (_s[id] == true) { _s[id] = false; notifyListeners(); } }
...
class ConnModel extends ChangeNotifier {
  ...
  void log(String line) {
    ...
    logs.insert(0, '$ts $line'); if (logs.length > 60) logs.removeLast(); notifyListeners();
  }
}
