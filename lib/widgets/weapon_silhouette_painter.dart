...
class WeaponSilhouettePainter extends CustomPainter {
  ...
  void _drawActive(Canvas canvas, Size size, Paint p) {
    ...
    switch (type) {
      case WeaponType.primary:
        _ak47Active(canvas, s, p);
        break;
      case WeaponType.pistol:
        _deagleActive(canvas, s, p);
        break;
      case WeaponType.grenade:
        _grenadeActive(canvas, s, p);
        break;
      case WeaponType.knife:
        _knifeActive(canvas, s, p);
        break;
    }
  }
  ...
  void _drawInactive(Canvas canvas, Size size, Paint p) {
    ...
    switch (type) {
      case WeaponType.primary:
        _ak47Inactive(canvas, s, p);
        break;
      case WeaponType.pistol:
        _deagleInactive(canvas, s, p);
        break;
      case WeaponType.grenade:
        _grenadeInactive(canvas, s, p);
        break;
      case WeaponType.knife:
        _knifeInactive(canvas, s, p);
        break;
    }
  }
  ...
  void _ak47Active(Canvas canvas, double s, Paint p) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: Offset(-s * 0.05, 0), width: s * 1.4, height: s * 0.18),
  ...
  void _ak47Inactive(Canvas canvas, double s, Paint p) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: Offset.zero, width: s * 1.6, height: s * 0.20),
          Radius.circular(s * 0.04),
        ),
        p);
    canvas.drawRRect(
  ...
  void _deagleInactive(Canvas canvas, double s, Paint p) {
    canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: Offset.zero, width: s * 1.0, height: s * 0.22),
          Radius.circular(s * 0.05),
        ),
        p);
    final gripPath = Path()
  ...
  void _grenadeActive(Canvas canvas, double s, Paint p) {
    ...
    canvas.drawOval(
      Rect.fromCenter(
          center: Offset(0, s * 0.05), width: s * 0.55, height: s * 0.70),
      p,
    );
    // Top neck
    canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: Offset(0, -s * 0.38), width: s * 0.18, height: s * 0.16),
          Radius.circular(s * 0.04),
        ),
        p);
    // Pin lever
  ...
  void _grenadeInactive(Canvas canvas, double s, Paint p) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: Offset(0, -s * 0.46), width: s * 0.18, height: s * 0.14),
          Radius.circular(s * 0.03),
        ),
        p);
  }
