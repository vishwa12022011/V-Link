...
import 'visual_engine_screen.dart';
...
class _NavBar extends StatelessWidget {
  ...
  const _NavBar({required this.selected, required this.onTap, required this.connStatus});

  @override
  Widget build(BuildContext context) {
    final connColor = connStatus == ConnStatus.connected ? T.teal
        : connStatus == ConnStatus.scanning ? T.gold : T.greyDim;
    ...
    return Container(
      ...
      child: Row(children: [
        // Left accent bar — connection colour
          ...
          return Expanded(child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: sel
                    ? const Border(top: BorderSide(color: T.red, width: 2))
                    : null,
              ),
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(_MainShellState._tabs[i].$1,
                    size: 19, color: sel ? T.red : T.greyDim),
                const SizedBox(height: 3),
                Text(_MainShellState._tabs[i].$2,
                    style: T.mono(8, color: sel ? T.red : T.greyDim)),
              ]),
            ),
          ));
        }),
