      ...
      body: CustomScrollView(slivers: [
        ...
        SliverToBoxAdapter(
            child: _Sec(
                'GLOBAL APPEARANCE',
                Column(children: [
                  _Slider('INTERFACE OPACITY', vm.opacity,
                      '${(vm.opacity * 100).round()}%', (v) {
                    final model = context.read<VisualModel>();
                    model.opacity = v;
                  }),
                  _Slider('GLOW INTENSITY', vm.glowIntensity,
                      '${(vm.glowIntensity * 100).round()}%', (v) {
                    final model = context.read<VisualModel>();
                    model.glowIntensity = v;
                  }),
                  _Slider('HUD SCALE', (vm.hudScale - 0.5) / 1.5,
                      '${vm.hudScale.toStringAsFixed(2)}×', (v) {
                    final model = context.read<VisualModel>();
